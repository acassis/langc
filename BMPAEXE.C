...
void main()
{
   ...
   if ((s=fopen(nome,"rt"))==NULL)
       ...
       if(ta>=0x77)
	{
	  ...
	  ctcol-=2;
	  printf("   %d   ",ctcol);
	   posy.linha =lin-(ctlin);
	   testacor(ch);
	   if(cor1!=0 || cor2!=0)
	    {
	     posx.coluna=col-(ctcol+2);
	     fprintf(f,"´º%c%c¹%c%c°%cÍ",posy.lin[0],posy.lin[1],posx.col[0],posx.col[1],cor2);
	     posx.coluna+=1;
	     fprintf(f,"´º%c%c¹%c%c°%cÍ",posy.lin[0],posy.lin[1],posx.col[0],posx.col[1],cor1);
	    }
	}
    }
